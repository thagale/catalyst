...
describe("dispatch skip attribution", () => {
  test("classifies known reasons and fails closed", () => {
    ...
    for (const reason of [null, "", "new-reason", "prior-artifact-retry-exhausted", 7])
      expect(classifyStallReason(reason)).toBe("unknown");
  });
  ...
  test("operator parks do not count as starvation", () => {
    expect(
      hasStarvingWork({
        readyIds: ["CAT-1"],
        skips: [{ ticket: "CAT-1", class: "operator-owned" }],
      })
    ).toBe(false);
    expect(
      hasStarvingWork({ readyIds: ["CAT-1"], skips: [{ ticket: "CAT-1", class: "machine-owned" }] })
    ).toBe(true);
    expect(
      hasStarvingWork({
        readyIds: ["CAT-1", "CAT-2"],
        skips: [{ ticket: "CAT-1", class: "operator-owned" }],
      })
    ).toBe(true);
