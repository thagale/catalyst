...
| `CATALYST_BH_TRIAGE_PRODUCTION` _(env var)_              | `shadow`          | Triage-production invariant mode: `off` skips triage-artifact inspection, `shadow` publishes stalled candidates without making the invariant actionable, and `enforce` reports free capacity plus owned eligible tickets missing `triage.json` when production is stale or a held-sweep event corroborates the outage. The move is escalate-only and cannot dispatch a recovery delegate. Garbage values fall back to `shadow`. Overrides Layer-2. |
| `catalyst.boardHealth.triageProduction` _(Layer-2)_       | `shadow`          | Same three values; honored when `CATALYST_BH_TRIAGE_PRODUCTION` is unset. |
| `CATALYST_BH_TRIAGE_STALL_MS`                            | `1800000` (30 min) | Maximum age of the most recent successful triage completion before owned, untriaged eligible work with free capacity is reported as stalled. |
