...
# A repo whose real auto-merge workflow is still suppressed, but which also holds
# an unrelated PAT-using helper that sorts EARLIER. Classifying on the first
# merge-performing workflow certifies the repo clean and hides the breakage this
# audit exists to find, so every merge-performing workflow must be weighed.
sed 's/secrets.GITHUB_TOKEN/secrets.RELEASE_PAT/' "$SCRATCH/fixtures/org/vanity/auto-merge.yml" >"$SCRATCH/fixtures/org/masked/a-approve-merge.yml"
cp "$SCRATCH/fixtures/org/vanity/auto-merge.yml" "$SCRATCH/fixtures/org/masked/z-auto-merge.yml"
cp "$SCRATCH/fixtures/org/vanity/ci.yml" "$SCRATCH/fixtures/org/masked/ci.yml"
cat >"$SCRATCH/gh" <<'EOF'
#!/usr/bin/env bash
set -u
root="${GH_FIXTURE_DIR:?}"
if [[ "$1" == api ]]; then
  ...
  if [[ -z "$rest" ]]; then
    ...
    find "$d" -type f -name '*.yml' -maxdepth 1 -print | sed "s|$d/|.github/workflows/|" | sort
  else
...
EOF
chmod +x "$SCRATCH/gh"
printf '%s\n' '{"org/vanity":"VAN","org/fixed":"FIX","org/none":"NON","org/masked":"MSK"}' >"$SCRATCH/repos.json"
set +e
...
check "an earlier PAT helper does not mask a suppressed auto-merge" contains "$SCRATCH/out" $'org/masked\tz-auto-merge.yml\tsecrets.GITHUB_TOKEN\tsuppressed'
...
check "JSON output parses" jq -e 'length == 4 and .[0].repo' "$SCRATCH/json"
