...
| `catalyst.blockedGhost.mode` _(Layer 2)_                     | `shadow`                     | Tier-1 liveness policy for a session listed by `claude agents --json` with `state:"blocked"` (CAT-171). `off` preserves presence-only liveness with no events; `shadow` preserves that verdict and emits one `liveness.blocked-ghost.observed` event per session/process; `enforce` classifies every listed blocked session as not alive and emits a deduplicated `phase.terminal.reap-requested` cleanup intent — which normally resolves to a `claude stop`, but escalates to a worktree-deleting `claude rm` when that stop no-ops and the session still lists as reapable (see `docs/architecture.md` for the full caveat; CAT-267 tracks narrowing it). Precedence is `CATALYST_BLOCKED_GHOST` (`0` is the kill switch) → this key → `shadow`; invalid values fall down the ladder and never escalate to `enforce`. The mode is resolved at daemon/runner boot, so changes require restart. Run `shadow` for at least 24 hours, confirm each observation against the Tier-2 job state, then roll out one host per day (aldebaran → vega → sophon) under an operator gate. |
