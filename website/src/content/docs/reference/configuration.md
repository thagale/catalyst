...
| `orchestration.stalledRepull.mode`                            | `shadow`                     | Re-pull behavior for eligible tickets held by a machine-owned terminal worker directory: `off` disables evaluation, `shadow` reports without mutation, and `enforce` detaches the directory before a fresh dispatch. Env `CATALYST_STALLED_REPULL` overrides this key. |
| `orchestration.stalledRepull.graceMs`                         | `1800000`                    | Minimum terminal worker-directory age before it is eligible for re-pull. |
| `orchestration.stalledRepull.maxRepullAttempts`               | `2`                          | Persistent per-ticket cap on automatic stalled re-pulls. |
| `orchestration.stalledRepull.repullBackoffMs`                 | `7200000`                    | Minimum delay between automatic re-pull attempts for the same ticket. |
...
| `CATALYST_PLUGIN_DIRTY_GUARD` _(env var)_                     | `enforce`                    | Controls whether plugin refresh refuses to `git reset --hard` over uncommitted tracked changes. `enforce` skips and emits `plugin.checkout.dirty_skipped`, escalating to `plugin.checkout.dirty_stale`; `shadow` emits `plugin.checkout.would_skip_dirty` and resets; `off` restores unconditional reset. Unknown values fail safe to `enforce`. Untracked files do not block refresh. (CAT-167)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `CATALYST_PLUGIN_DIRTY_SKIP_GRACE_MS` _(env var)_             | `1800000`                    | Time a checkout may remain blocked by tracked dirt before the one-shot `plugin.checkout.dirty_stale` ERROR. A successful refresh clears the episode. (CAT-167)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `orchestration.replicaSweep.enabled`                          | `true`                       | Sample local Linear replica completeness into `<orchDir>/replica-state.json`. |
| `orchestration.replicaSweep.intervalSeconds`                  | `300`                        | Replica completeness sampling cadence. |
...
`CATALYST_REPLICA_RESEED` controls the boot reseed request (`off`, `shadow`, or `enforce`; default
`shadow`). `CATALYST_REPLICA_RESEED_COOLDOWN_MS` defaults to 3600000. The Layer-2 equivalent is
`catalyst.linearReplica.reseed`.
...
`CATALYST_BH_REPLICA` independently controls the replica completeness invariant (`off`, `shadow`,
or `enforce`; default `shadow`).
...
| `CATALYST_BH_UNOWNED_INFLIGHT_MS`                       | `86400000` (24 h)   | Stale-unowned threshold (CTL-1475). A Linear state like `Implement` is a **claim** that a worker is on the ticket, not a label — and nothing takes the claim back when the worker dies. Past this age with **no live worker signal and no confirmed-open PR**, the ticket is flagged `unownedInFlight` and proposed as a **tier2 (anchorable)** `recover-unowned-in-flight` move, so the delegate dispatches a recovery pass rather than merely reporting it. “No confirmed-open PR” is established by the authoritative union of ticket-key search, branch-head search, and Linear PR attachments, not only the webhook cache. Such tickets are invisible to every other path: admission only pulls `Todo`, and the recovery census scans worker dirs they have no entry in. Deliberately conservative — any evidence of ownership spares the ticket, since a false negative costs one more scan while a false positive re-dispatches work a human is holding. Actuation remains operator-gated by `CATALYST_BOARD_HEALTH=enforce`. |
| `CATALYST_BH_UNOWNED_PR_VERIFY`                         | enabled             | Kill switch for authoritative open-PR verification of stale unowned candidates. Set `0` to disable the live union check. This does not enable actuation; `CATALYST_BOARD_HEALTH=enforce` is still required. |
| `CATALYST_BH_UNOWNED_PR_VERIFY_MAX`                     | `5`                 | Maximum stale-unowned candidates verified per board scan, bounding GitHub API cost. Overflow candidates are conservatively left unconfirmed for a later scan. |
| `CATALYST_BH_UNOWNED_PR_VERIFY_TTL_MS`                  | `1800000` (30 min)  | Memo lifetime for an authoritative open-PR result. The recovery delegate re-verifies before acting because its brief may contain a cached snapshot this old. |
| `CATALYST_BH_UNOWNED_PR_MIN_QUOTA`                      | `500`               | Minimum remaining GitHub core REST requests required before live verification proceeds. A known lower balance makes the check unverifiable and spares the candidate; an absent snapshot is advisory and does not disable verification. |
